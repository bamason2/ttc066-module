...
Change your model so it looks like the one below – you’ll need to find the product, and the (simple) ‘1-D Lookup Table’ block in the Simulink library, and drag and drop them onto your model.
...
This is trickier, as the gear change timing needs to be optimised, by trial and error. What I’m trying to achieve is maximum use of first gear (where the torque is always higher than second gear), but not holding first gear for too long (as the vehicle can’t get to 60mph in first gear). Here is some code that I used, progressively changing gct12. Try higher and lower values, and you’ll see in both cases the time to 26.67m/s is longer.. :
