...
The quarter-car ride model is illustrated as shown, with two masses $M_1$ and $M_2$ representing the sprung mass (vehicle body) and unsprung mass (wheel) associated with a single suspension corner of the vehicle.
...
The lower spring ($K_2$) represents the compliance of the rubber tyre, and the upper spring ($K_1$) is the main suspension spring (e.g. a coil spring). The damper (shock absorber) B provides a force proportional to the relative velocity between $M_1$ and $M_2$.
...
and highlight all of these blocks, then select create a subsystem from the Diagram menu. Then double click the subsystem to re-open it, and after re-naming the inport, outport and Sub-system name you will simply have:
...
<img src="figs/suspension_fig6.png" width="700"/>\\
*Figure: A new model*

Complete this sub-system, using the linear gains for suspension spring and damper rates given on the front page. This effectively amounts to wiring up the first equation you derived in Step 1. You will only need to use in, out, gain and sum blocks (Commonly Used Blocks) to do this. When you’re finished, highlight all blocks and Create Subsystem as before. Save this as suspension.slx.
